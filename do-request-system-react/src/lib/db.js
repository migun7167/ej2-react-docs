import { buildDocChecklist } from "./domain.js";

export const LS_KEY = "aot_do_demo_react_v2";
...
// Documents live on the shipment (not the request) so TMO can scan/attach
// them as soon as cargo arrives — independent of whether, or when, a
// Forwarder later requests a DO for it.
function buildShipments() {
  const base = [
    ...
    { id: "s13", mawb: "217-30099001", hawb: null, type: "Master (Direct)", consignee: "Kerry Logistics (Thailand) Co., Ltd.", consigneeTaxId: "0105536090809", flight: "TG920", flightDate: "2026-07-04", origin: "NRT", destination: "BKK", pieces: 10, weight: "300 KG", goods: "Automotive Electronics", customsStatus: "CLEARED", cargoStatus: "AVAILABLE", storage: "AOT Cargo Terminal - Zone A/14", doStatus: null },
  ];

  const shipments = base.map((s) => ({ ...s, docChecklist: buildDocChecklist(s) }));

  const attach = (id, updates) => {
    const s = shipments.find((x) => x.id === id);
    updates.forEach(([type, patch]) => {
      const doc = s.docChecklist.find((d) => d.type === type);
      Object.assign(doc, patch);
    });
  };

  // s6, s9, s12 back existing DO_ASSIGNED history — their required docs were
  // scanned before/around when those DOs were approved.
  attach("s6", [
    ["เอกสารแนบเครื่อง (Flight Document)", { state: "attached", scannedBy: "สมชาย ใจดี", scannedAt: hoursAgo(70) }],
    ["MAWB Copy", { state: "attached", scannedBy: "สมชาย ใจดี", scannedAt: hoursAgo(70) }],
  ]);
  attach("s9", [
    ["เอกสารแนบเครื่อง (Flight Document)", { state: "attached", scannedBy: "สุดา วงศ์ทอง", scannedAt: hoursAgo(148) }],
    ["MAWB Copy", { state: "attached", scannedBy: "สุดา วงศ์ทอง", scannedAt: hoursAgo(148) }],
    ["HAWB Copy", { state: "attached", scannedBy: "สุดา วงศ์ทอง", scannedAt: hoursAgo(147) }],
    ["Cargo Manifest / Breakdown Sheet", { state: "attached", scannedBy: "สุดา วงศ์ทอง", scannedAt: hoursAgo(147) }],
  ]);
  attach("s12", [
    ["เอกสารแนบเครื่อง (Flight Document)", { state: "attached", scannedBy: "สมชาย ใจดี", scannedAt: hoursAgo(9.5) }],
    ["MAWB Copy", { state: "attached", scannedBy: "สมชาย ใจดี", scannedAt: hoursAgo(9.5) }],
  ]);
  // s8 — flight document scanned but unreadable, feeding the seeded MANUAL_REVIEW request.
  attach("s8", [["เอกสารแนบเครื่อง (Flight Document)", { state: "unreadable", scannedBy: "สมชาย ใจดี", scannedAt: hoursAgo(4) }]]);
  // s11 — fully attached, feeding the seeded READY_FOR_APPROVAL request.
  attach("s11", [
    ["เอกสารแนบเครื่อง (Flight Document)", { state: "attached", scannedBy: "สุดา วงศ์ทอง", scannedAt: hoursAgo(15) }],
    ["MAWB Copy", { state: "attached", scannedBy: "สุดา วงศ์ทอง", scannedAt: hoursAgo(15) }],
  ]);
  // s13 — TMO pre-attached documents before any Forwarder has requested a DO
  // for it, demonstrating that a later request can skip straight to decision.
  attach("s13", [
    ["เอกสารแนบเครื่อง (Flight Document)", { state: "attached", scannedBy: "สมชาย ใจดี", scannedAt: hoursAgo(2) }],
    ["MAWB Copy", { state: "attached", scannedBy: "สมชาย ใจดี", scannedAt: hoursAgo(2) }],
  ]);
...
export function seedDB() {
  return {
    ...
    requests: [
      { id: "req_seed_s9", requesterId: "fwd2", requesterName: "Somsak Deelert", requesterCompany: "DHL Global Forwarding (Thailand) Ltd.", requesterTaxId: "0105537083550", mawb: "217-30044550", hawb: "THB-771133", shipmentId: "s9", entitlement: "PASS_HOUSE", status: "DO_ASSIGNED",
      ...
      { id: "req_seed_s6", requesterId: "fwd1", requesterName: "วราภรณ์ พงษ์เจริญ", requesterCompany: "Kerry Logistics (Thailand) Co., Ltd.", requesterTaxId: "0105536090809", mawb: "217-30011220", hawb: null, shipmentId: "s6", entitlement: "PASS_MASTER", status: "DO_ASSIGNED",
      ...
      { id: "req_seed_s10", requesterId: "fwd3", requesterName: "Nattapong Srisuk", requesterCompany: "ABC Cargo Services Co., Ltd.", requesterTaxId: "0105558123456", mawb: "217-30055660", hawb: null, shipmentId: "s10", entitlement: "FAIL", status: "ENTITLEMENT_FAILED",
      ...
      { id: "req_seed_s8", requesterId: "fwd2", requesterName: "Somsak Deelert", requesterCompany: "DHL Global Forwarding (Thailand) Ltd.", requesterTaxId: "0105537083550", mawb: "217-30033440", hawb: "THB-771122", shipmentId: "s8", entitlement: "PASS_HOUSE", status: "MANUAL_REVIEW",
      ...
      { id: "req_seed_s11", requesterId: "fwd3", requesterName: "Nattapong Srisuk", requesterCompany: "ABC Cargo Services Co., Ltd.", requesterTaxId: "0105558123456", mawb: "217-30066770", hawb: null, shipmentId: "s11", entitlement: "PASS_DELEGATED", status: "READY_FOR_APPROVAL",
      ...
      { id: "req_seed_s12", requesterId: "fwd1", requesterName: "วราภรณ์ พงษ์เจริญ", requesterCompany: "Kerry Logistics (Thailand) Co., Ltd.", requesterTaxId: "0105536090809", mawb: "217-30077880", hawb: null, shipmentId: "s12", entitlement: "PASS_MASTER", status: "DO_ASSIGNED",
      ...
      { id: "req_seed_s7", requesterId: "fwd1", requesterName: "วราภรณ์ พงษ์เจริญ", requesterCompany: "Kerry Logistics (Thailand) Co., Ltd.", requesterTaxId: "0105536090809", mawb: "217-30022330", hawb: null, shipmentId: "s7", entitlement: "PASS_MASTER", status: "PENDING_DOCUMENT_SCAN",
    ...
    auditLog: [
      ...
      { id: "log_seed_4", at: hoursAgo(147), userId: "tmo2", userName: "สุดา วงศ์ทอง", role: "TMO", action: "DOCUMENT_SCANNED", meta: { requestId: "req_seed_s9", shipmentId: "s9", doc: "HAWB Copy", state: "attached" } },
      ...
      { id: "log_seed_9", at: hoursAgo(70), userId: "tmo1", userName: "สมชาย ใจดี", role: "TMO", action: "DOCUMENT_SCANNED", meta: { requestId: "req_seed_s6", shipmentId: "s6", doc: "เอกสารแนบเครื่อง (Flight Document)", state: "attached" } },
      ...
      { id: "log_seed_12", at: hoursAgo(4), userId: "tmo1", userName: "สมชาย ใจดี", role: "TMO", action: "DOCUMENT_SCANNED", meta: { requestId: "req_seed_s8", shipmentId: "s8", doc: "เอกสารแนบเครื่อง (Flight Document)", state: "unreadable" } },
      ...
      { id: "log_seed_16", at: hoursAgo(15), userId: "tmo2", userName: "สุดา วงศ์ทอง", role: "TMO", action: "DOCUMENT_SCANNED", meta: { requestId: "req_seed_s11", shipmentId: "s11", doc: "เอกสารแนบเครื่อง (Flight Document)", state: "attached" } },
      ...
      { id: "log_seed_24", at: hoursAgo(2), userId: "tmo1", userName: "สมชาย ใจดี", role: "TMO", action: "DOCUMENT_SCANNED", meta: { shipmentId: "s13", doc: "เอกสารแนบเครื่อง (Flight Document)", state: "attached" } },
      { id: "log_seed_25", at: hoursAgo(2), userId: "tmo1", userName: "สมชาย ใจดี", role: "TMO", action: "DOCUMENT_SCANNED", meta: { shipmentId: "s13", doc: "MAWB Copy", state: "attached" } },
    ],
  };
}
